...
const Stacks = () => {
    ...
    const mapTechs = technologies.map((tech, index) => {
        return (
            <button className={index === num ? "bg-blue text-white rounded-full w-auto flex-nowrap capitalize  px-10 font-bold" : " " + `p-4 mx-1 hover:bg-light-blue hover:text-blue rounded-full flex-nowrap text-black px-10 capitalize`} key={index}>{tech}</button>
        )
    })
    const mapStacks = stacks[num].map((stack, index) => {
        return (
            <div key={index} className="lg:w-52 sm:w-1/2 my-10 sm:my-4">
    ...
    return (
        <div>
            <div className="py-6">
                ...
                <div className="flex flex-wrap sm:w-full w-9/12  mx-auto justify-between px-10">
                    {mapStacks}
                </div>
            </div>
        </div>
